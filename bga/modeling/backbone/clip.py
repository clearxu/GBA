...
from .adapter import ContextInteraction, MonaOp_st, MonaOp_sem
...
class CLIP(Backbone):
    def __init__(self, cfg, input_shape):
        ...
        pretrained = '/data/changwei/.cache/huggingface/hub/models--laion--CLIP-convnext_large_d_320.laion2B-s29B-b131K-ft-soup/snapshots/4070cadc6220ffa4df32772e528ec11e7cb73780/open_clip_pytorch_model.bin'
        ...
        self.mona_st = MonaOp_st(in_features=192)
        self.mona_sem = MonaOp_sem(in_features=1536)
    ...
    def extract_features(self, x,text_classifier):
        return {
            'convnext': self.extract_features_convnext,
            'resnet': self.extract_features_resnet,
        }[self.model_type](x,text_classifier)
    ...
    def extract_features_convnext(self, x, text_classifier):
        out = {}
        ...
        for i in range(4):
            ...
            if i == 0:
                ...
                with torch.enable_grad():
                    x = self.mona_sem(x,text_classifier)
                    out[f'res{i + 2}'] = x
    ...
    def forward(self, x,text_classifier):
        self.eval()
        with torch.no_grad():
